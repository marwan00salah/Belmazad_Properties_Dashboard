...
export function renderListings() {
  ...
  root.className = "flex-1 mx-auto w-full max-w-7xl px-4 md:px-6 py-4 md:py-6 space-y-4";
  ...
  if (error && listings.length === 0) {
    ...
    err.className = "mx-auto max-w-md text-center py-16";
    err.innerHTML = `
      <div class="text-ink-900 text-lg font-semibold mb-1">Couldn't reach the server</div>
      <p class="text-ink-500 text-sm mb-4">${error}</p>
      <button id="retry-btn" class="inline-flex items-center gap-2 rounded-lg bg-accent-700 hover:bg-accent-800 text-white px-4 py-2 text-sm font-medium transition shadow-sm">Retry</button>`;
  ...
  meta.textContent = `Showing ${filtered.length} of ${listings.length} listing${listings.length === 1 ? "" : "s"}`;
  root.appendChild(meta);

  if (filtered.length === 0) {
    const empty = document.createElement("div");
    empty.className = "mx-auto max-w-md text-center py-16";
    empty.innerHTML = `
      <div class="text-ink-900 text-lg font-semibold mb-1">No listings match these filters</div>
      <p class="text-ink-500 text-sm mb-4">Try widening your search or clearing the filters.</p>
      <button id="clear-btn" class="inline-flex items-center gap-2 rounded-lg bg-white hover:bg-ink-50 text-ink-800 ring-1 ring-ink-200 px-4 py-2 text-sm font-medium transition shadow-sm">Clear filters</button>`;
  ...
  grid.className = "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 fade-in";
...
function renderSignInPanel() {
  ...
  wrap.className = "mx-auto max-w-md text-center py-16";

  const signInUrl = `${WORKER_URL}auth-return?dest=${encodeURIComponent(window.location.href)}`;

  wrap.innerHTML = `
    <svg width="40" height="40" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" class="mx-auto mb-3 text-accent-700">
    ...
    <a id="signin-btn" href="${signInUrl}"
       class="inline-flex items-center gap-2 rounded-lg bg-accent-700 hover:bg-accent-800 text-white px-5 py-2 text-sm font-semibold shadow-sm transition">
